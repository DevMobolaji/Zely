...
const TransfersScreen: React.FC = () => {
    ...
    const getSourceAccounts = () => accountsData.filter(acc => acc.type === 'current');
    const getDestAccounts = () => accountsData.filter(acc => acc.type === 'savings');
    ...
    const handleFunding = async () => {
        ...
        try {
            ...
            if (!scriptLoaded) throw new Error('Failed to load payment provider script');
            ...
            const handler = (window as any).PaystackPop.setup({
                ...
                callback: (response: any) => {
                    ...
                    showToast('success', `Funding successful! Reference: ${response.reference}`);
                    setFundingAmount('');
                },
                onClose: () => {
                    setFundStatus('idle');
            ...
            <div className="bg-white dark:bg-slate-900 rounded-[2rem] p-8 border border-slate-200 dark:border-slate-800 shadow-xl min-h-[500px] flex flex-col">
                ...
                {transferStatus !== 'success' && transferStatus !== 'confirm' && (
                    <div className="relative flex p-1 bg-slate-100 dark:bg-slate-800 rounded-xl mb-8">
                        <div
                            className={`absolute inset-y-1 w-[calc(50%-4px)] bg-white dark:bg-slate-700 rounded-lg shadow-sm transition-all duration-300 ease-out ${transferType === 'p2p' ? 'translate-x-[calc(100%+4px)]' : 'translate-x-1'
                                }`}
                        />
                        <button
                ...
                {transferStatus === 'success' ? (
                    ...
                    <div className="space-y-6 flex-1">
                        ...
                        <div>
                            ...
                            {transferStatus === 'idle' && (
                                <div className="mt-4">
                                    <p className="text-[10px] font-bold text-slate-400 uppercase tracking-widest mb-2">Suggested Amounts</p>
                                    <div className="flex flex-wrap gap-2">
                                        {SUGGESTED_AMOUNTS.map((val) => (
                                            <button
                                                ...
                                                className={`px-4 py-2 rounded-xl text-xs font-bold border transition-all ${amount === val.toString()
