...
const DashboardScreen: React.FC = () => {
    ...
    const totalBalance = accountsData.reduce((acc, curr) => acc + curr.balance, 0);
    ...
    return (
        <div className="w-full max-w-5xl mx-auto space-y-8 animate-in fade-in slide-in-from-bottom-4 duration-500">
            ...
            <div className="flex flex-col sm:flex-row sm:items-end justify-between gap-4">
                ...
                <button onClick={() => navigate('/fund-wallet')} className="hidden sm:flex items-center gap-2 px-5 py-2.5 bg-slate-900 dark:bg-white text-white dark:text-slate-900 rounded-xl font-bold hover:scale-105 transition-transform shadow-lg shadow-slate-200 dark:shadow-none text-sm">
            ...
            <div className="relative overflow-hidden rounded-[2rem] p-8 shadow-xl">
                <div className="absolute inset-0 bg-gradient-to-br from-slate-900 to-slate-800 dark:from-slate-800 dark:to-black"></div>
                ...
                <div className="relative z-10 flex flex-col md:flex-row justify-between items-start md:items-end gap-8">
                    <div>
                        <div className="flex items-center gap-2 mb-2">
                            ...
                            <div className="px-2 py-0.5 rounded-full bg-white/10 text-white/80 text-[10px] font-bold backdrop-blur-sm">NGN</div>
                        </div>
                    ...
                    <div className="flex items-center gap-3 w-full md:w-auto">
                        <button onClick={() => navigate('/transfers')} className="flex-1 md:flex-none flex items-center justify-center gap-2 px-6 py-3.5 bg-white text-slate-900 rounded-xl font-bold hover:bg-slate-50 transition-colors shadow-lg">
                            <Send className="w-4 h-4" /> Transfer
                        </button>
                        <button onClick={() => navigate('/savings')} className="flex-1 md:flex-none flex items-center justify-center gap-2 px-6 py-3.5 bg-white/10 text-white backdrop-blur-sm border border-white/10 rounded-xl font-bold hover:bg-white/20 transition-colors">
                ...
                <div className="lg:col-span-2 space-y-8">
                    ...
                    {recentContacts.length > 0 && (
                        <div>
                            ...
                            <div className="flex gap-4 overflow-x-auto p-1 pb-4 no-scrollbar">
                                ...
                                >
                                    <div className="w-14 h-14 rounded-full bg-slate-100 dark:bg-slate-800 border-2 border-dashed border-slate-300 dark:border-slate-700 flex items-center justify-center text-slate-400 group-hover:border-primary group-hover:text-primary transition-colors">
                                ...
                                {recentContacts.map((contact, idx) => (
                                    <button
                                        ...
                                        className="flex flex-col items-center gap-2 min-w-[80px] group"
                                    >
                                        <div className="w-14 h-14 rounded-full bg-slate-200 dark:bg-slate-700 overflow-hidden ring-2 ring-transparent group-hover:ring-primary transition-all relative">
                                            {contact.avatar ? (
                    ...
                    <div>
                        ...
                        <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
                            {/* Main Checking Card */}
                            <div
                                onClick={() => navigate(`/wallets/${accountsData[0].id}`)}
                                className="bg-slate-900 text-white rounded-[1.5rem] p-6 relative overflow-hidden shadow-xl group cursor-pointer transition-all hover:-translate-y-1 hover:shadow-2xl"
                            >
                                <div className="absolute top-0 right-0 w-64 h-64 bg-white/5 rounded-full blur-3xl -translate-y-1/2 translate-x-1/2"></div>
                                <div className="relative z-10 flex flex-col justify-between h-40">
                                    <div className="flex justify-between items-start">
                                        <span className="font-medium text-slate-400 text-sm">Main Checking</span>
                                        <span className="font-black italic text-xl tracking-widest opacity-80">VISA</span>
                                    </div>
                                    <div>
                                        <h3 className="text-2xl font-bold tracking-tight mb-1">₦{accountsData[0].balance.toLocaleString('en-NG')}</h3>
                                        <p className="font-mono text-slate-400 tracking-wider text-xs">•••• •••• •••• {accountsData[0].cardLast4}</p>
                                    </div>
                                    <div className="flex justify-between items-center text-xs font-medium text-slate-400">
                                        <span className="uppercase">{userName || 'User'}</span>
                                        <span>{accountsData[0].cardExpiry}</span>
                                    </div>
                                </div>
                            </div>
                            ...
                            {/* Savings Card */}
                            <div
                                onClick={() => navigate(`/wallets/${accountsData[1].id}`)}
                                className="bg-white dark:bg-slate-800 text-slate-900 dark:text-white rounded-[1.5rem] p-6 border border-slate-200 dark:border-slate-700 relative overflow-hidden shadow-sm group cursor-pointer transition-all hover:-translate-y-1 hover:shadow-md"
                            >
                                <div className="absolute -bottom-10 -right-10 w-40 h-40 bg-green-500/10 rounded-full blur-2xl"></div>
                                <div className="relative z-10 flex flex-col justify-between h-40">
                                    <div className="flex justify-between items-start">
                                        <span className="font-medium text-slate-500 dark:text-slate-400 text-sm">High-Yield Savings</span>
                                        <div className="w-8 h-5 bg-slate-200 dark:bg-slate-600 rounded opacity-50"></div>
                                    </div>
                                    <div>
                                        <h3 className="text-2xl font-bold tracking-tight mb-1">₦{accountsData[1].balance.toLocaleString('en-NG')}</h3>
                                        <div className="inline-flex items-center gap-1.5 px-2.5 py-0.5 bg-green-100 dark:bg-green-900/20 text-green-700 dark:text-green-400 rounded-full text-[10px] font-bold">
                                            <ArrowUpRight className="w-3 h-3" />
                                            {accountsData[1].trend} APY
                                        </div>
                                    </div>
                                    <div className="flex justify-between items-center text-xs font-medium text-slate-500">
                                        <span>•••• {accountsData[1].cardLast4}</span>
