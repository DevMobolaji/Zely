...
import { Wallet, CreditCard, ArrowUpRight, ArrowDownLeft, Plus, ChevronRight, Copy, History } from 'lucide-react';
            ...
            <div className="w-full max-w-4xl mx-auto space-y-6 animate-in fade-in slide-in-from-right-4">
                    ...
                    <div className="relative z-10">
                        <div className="flex justify-between items-start mb-6">
                            ...
                            <div className="flex items-center gap-2">
                                <span className="px-3 py-1 bg-white/10 rounded-full text-xs font-bold uppercase tracking-wider">{wallet.type}</span>
                            </div>
                        </div>
                        <h2 className="text-slate-400 font-bold mb-1">{wallet.name}</h2>
                        <h1 className="text-4xl font-black mb-6">{wallet.currency}{wallet.balance.toLocaleString('en-NG')}</h1>
                        
                        <div className="flex gap-3">
                            <button onClick={() => navigate('/fund-wallet')} className="flex-1 py-3 bg-white text-slate-900 rounded-xl font-bold hover:bg-slate-100 transition-colors flex items-center justify-center gap-2">
                                <ArrowDownLeft className="w-4 h-4" /> Fund
                            </button>
                            <button onClick={() => navigate('/transfers')} className="flex-1 py-3 bg-white/10 text-white rounded-xl font-bold hover:bg-white/20 transition-colors flex items-center justify-center gap-2">
                ...
                <div className="bg-white dark:bg-slate-900 rounded-2xl border border-slate-100 dark:border-slate-800 shadow-sm overflow-hidden">
                    <div className="p-6 border-b border-slate-100 dark:border-slate-800 flex items-center justify-between">
                        <h3 className="font-bold flex items-center gap-2"><History className="w-5 h-5 text-slate-400" /> Recent Transactions</h3>
                    </div>
                    <div>
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
                {accounts.map((acc) => (
                    <div 
                        ...
                        className="group relative bg-white dark:bg-slate-900 rounded-2xl p-6 border border-slate-100 dark:border-slate-800 shadow-sm hover:shadow-lg transition-all duration-300 overflow-hidden cursor-pointer"
                    >
                        <div className="absolute top-0 right-0 w-32 h-32 bg-slate-50 dark:bg-slate-800/50 rounded-full blur-2xl -translate-y-1/2 translate-x-1/2 group-hover:bg-primary/5 transition-colors"></div>
                        
                        <div className="relative z-10">
                            <div className="flex justify-between items-start mb-8">
                                <div className={`w-12 h-12 rounded-xl flex items-center justify-center ${
                                    acc.type === 'crypto' ? 'bg-indigo-100 text-indigo-600 dark:bg-indigo-900/20 dark:text-indigo-400' : 
                                    acc.type === 'savings' ? 'bg-green-100 text-green-600 dark:bg-green-900/20 dark:text-green-400' :
                                ...
                                <div className="text-right">
                                    <span className={`text-[10px] font-bold uppercase px-2 py-1 rounded-lg ${
                                        acc.type === 'virtual' ? 'bg-purple-100 text-purple-600 dark:bg-purple-900/20 dark:text-purple-400' : 'bg-slate-100 text-slate-600 dark:bg-slate-800 dark:text-slate-400'
                                    }`}>
                                        {acc.type}
                                    </span>
                                </div>
