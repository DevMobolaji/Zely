  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      if (email) {
        ...
        if (res.ok && res.user) {
          // Store tokens and set auth state properly
          setAuthFromUser(res.user);

          showToast("success", "Email verified successfully!");

          // New users go to provisioning, returning users go to target
    ...
    <>
      ...
      <div className="space-y-4 text-center sm:text-left">
        ...
        <p className="text-lg text-gray-400 font-medium max-w-md leading-relaxed">
          Help us keep your account secure by verifying your identity through
          ...
          <div className="flex justify-center flex-col items-center gap-4 w-full">
            ...
            {mode === "email" && (
              <div className="flex items-center gap-2 text-xs font-bold text-slate-400">
                <RefreshCw
                  className={`w-3 h-3 ${countdown > 0 ? "" : "text-primary"}`}
                />
                {countdown > 0 ? (
