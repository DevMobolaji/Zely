...
import { ShieldCheck, Loader2, ArrowLeft, Key, Mail, AlertCircle, RefreshCw } from 'lucide-react';
...
const TwoFactorScreen: React.FC = () => {
    ...
    const target = location.state?.target || '/dashboard';
    const defaultMode = location.state?.mode || 'totp';
    ...
    const [mode, setMode] = useState<'totp' | 'backup' | 'email'>(defaultMode);
    ...
    const handleSubmit = async (e: React.FormEvent) => {
        ...
        try {
            if (email) {
                const res = await authService.verify(code, email);

                if (res && res.user) {
                    const data = res.user;
                    const { role: rl, email: Email } = data;

                    if (res.ok) {
                        showToast('success', 'Verification successful!');
                    }

                    // 1. Store Role in LocalStorage
                    localStorage.setItem('userRole', rl);
                    localStorage.setItem('userEmail', Email);

                    // 2. Update Auth Context
                    setAuth({ accessToken: data.accessToken, user: { role: rl, email: Email } });
                ...
                setError('Verification failed: Email context missing.');
            }
        } catch (err: any) {
                        ...
                        {mode === 'email' && (
                            <div className="flex items-center gap-2 text-xs font-bold text-slate-400">
                                ...
                                {countdown > 0 ? `Code expires in ${formatTime(countdown)}` : (
                                    <button type="button" onClick={() => setCountdown(100)} className="text-primary hover:underline">
