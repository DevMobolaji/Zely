...
const RegisterScreen: React.FC = () => {
  ...
  const handleGoogleSignup = async () => {
    showToast("error", "Google sign-up is not available yet.");
  };
    ...
    <>
      ...
      <div className="space-y-4 text-center sm:text-left">
        ...
        <p className="text-lg text-gray-400 font-medium max-w-md leading-relaxed">
          Start your journey with us today. Get a verified ledger-backed account
          in minutes.
        </p>
      </div>
    </>
        ...
        <div>
          ...
          <div className="relative group">
            <div className="absolute inset-y-0 left-0 pl-3.5 flex items-center pointer-events-none">
              <User
                className={`w-5 h-5 transition-colors duration-300 ${errors.name ? "text-red-400" : "text-slate-400 group-focus-within:text-primary"}`}
              />
            </div>
            <input
              id="name"
              ...
              <div className="space-y-1">
                {requirements.map((req) => (
                  <div key={req.id} className="flex items-center gap-2">
                    <div
                      className={`w-3.5 h-3.5 rounded-full border flex items-center justify-center ${req.met ? "bg-green-500 border-green-500 text-white" : "border-slate-300 dark:border-slate-600"}`}
                    >
                      {req.met && <Check className="w-2 h-2" strokeWidth={4} />}
                    </div>
                    <span
                      className={`text-[10px] ${req.met ? "text-slate-700 dark:text-slate-200 font-bold" : "text-slate-400"}`}
                    >
                      {req.label}
                    </span>
                  </div>
