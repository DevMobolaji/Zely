...
import { Mail, Lock, Unlock, Loader2, AlertCircle, ArrowLeft } from 'lucide-react';
...
const LoginScreen: React.FC = () => {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      const nextTarget = role === 'admin' ? '/admin' : '/dashboard';
      
      // 2FA FLOW BYPASSED AS REQUESTED
      /*
      const { require2FA } = response;
      
      if (require2FA) {
          // Store temporary email for 2FA verify
          localStorage.setItem('pendingEmail', email);
          localStorage.setItem('userRole', role);
          navigate('/verify', { state: { target: nextTarget, mode: 'totp', email, role, isNewUser: false } });
      } else {
          const mockAccessToken = 'mock_access_' + Date.now();
          localStorage.setItem('userRole', role);
          setAuth({ accessToken: mockAccessToken, user: { role, email} });
      ...
      */
      ...
      const msg = error.response?.data?.message || 'Connection failed. Please check your credentials.';
      showToast('error', msg);
    } finally {
      setIsLoading(false);
    }
  ...
  const handleGoogleLogin = async () => {
    ...
    try {
      ...
      localStorage.setItem('userRole', role);
      setAuth({ accessToken: mockAccessToken, user: { role, email: "demo@zely.com" } });
  ...
  return (
    <AuthLayout rightSection={RightSectionContent}>
      ...
      <form onSubmit={handleSubmit} className="space-y-5" noValidate>
        ...
        <div>
          <div className="flex items-center justify-between mb-2">
            ...
            <button type="button" onClick={() => navigate('/reset-password')} className="text-sm font-semibold text-primary hover:text-primary-light transition-colors">
              Forgot Password?
            </button>
          </div>
          <div className="relative group">
      ...
      <p className="text-center text-sm text-slate-600 dark:text-slate-400 font-medium">
        Don't have an account? <button onClick={() => navigate('/register')} className="font-bold text-primary hover:underline">Sign Up</button>
      </p>
    </AuthLayout>
  );
};
