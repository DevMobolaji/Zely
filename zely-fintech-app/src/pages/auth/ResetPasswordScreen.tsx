...
import { Loader2, ArrowLeft, Lock, Unlock, Check, AlertCircle, RefreshCw, Key } from 'lucide-react';
...
const ResetPasswordScreen: React.FC = () => {
    ...
    const handleEmailSubmit = async (e: React.FormEvent) => {
        ...
        try {
            ...
            if (response && response.token) {
                showToast('success', response.token.message);
            }
            setStep('otp');
        } catch (error: any) {
            showToast('error', error.response?.data?.message || 'Failed to send reset code.');
        }
        setIsLoading(false);
    };
    ...
    const handleResendCode = async () => {
        try {
            const response = await authService.resetPasswordRequest(email)
            if (response && response.token) {
                showToast('success', response.token.message || 'Security update: A new verification code has been dispatched to your inbox.');
            }
            setCountdown(120)
            setIsLoading(false);
        } catch (error: any) {
            console.log(error)
            showToast('error', error.response?.data?.message || 'Failed to resend code.');
            setIsLoading(false);
            // setCountdown(120)
        }
    };
    ...
    const handlePasswordSubmit = async (e: React.FormEvent) => {
        ...
        showToast('success', 'Your password has been reset successfully! Redirecting...');
