  ...
  const handleEmailSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      if (errData?.code === "RATE_LIMIT_EXCEEDED") {
        setRateLimitCountdown(errData.retryAfter);
        showToast(
          "error",
  ...
  const handlePasswordSubmit = async (e: React.FormEvent) => {
    ...
    try {
      const response = await authService.resetPasswordConfirm({
        email,
        token: resetToken, // ← use the real reset token
    ...
    <>
      ...
      <div className="space-y-4 text-center sm:text-left">
        ...
        <p className="text-lg text-gray-400 font-medium max-w-md leading-relaxed">
          No worries, it happens. We'll help you get back into your account in
      ...
      <div className="flex flex-col animate-in fade-in slide-in-from-right-4 duration-300">
              ...
              <div>
                ...
                <div className="relative">
                  <input
                    ...
                    className={`w-full bg-slate-50 dark:bg-slate-800 border transition-all duration-300 rounded-xl py-3.5 px-4 text-slate-900 dark:text-white focus:outline-none focus:ring-2 ${
                      emailError
        ...
        {step === "otp" && (
          <div className="flex flex-col items-center">
            <h1 className="text-3xl font-black text-slate-900 dark:text-white mb-2 tracking-tight text-center">
              Security Check
            </h1>
            <p className="text-slate-500 dark:text-slate-400 mb-10 text-center font-medium leading-relaxed">
              For your protection, enter the 6-digit code we sent to
              <br />
            ...
            >
              <div className="relative w-full flex justify-center">
                <input
                  ...
                  className={`w-full max-w-[200px] text-center text-lg font-mono font-bold tracking-[0.4em] py-2 bg-slate-50 dark:bg-slate-800 border rounded-xl focus:outline-none focus:ring-4 focus:ring-primary/10 transition-all text-slate-900 dark:text-white ${
                    error
              ...
              {rateLimitCountdown !== null && (
                <div className="w-full p-4 bg-red-50 dark:bg-red-900/20 border border-red-200 dark:border-red-800 rounded-2xl text-center">
              ...
              <button
                type="button"
                disabled={
                  countdown > 0 || isLoading || rateLimitCountdown !== null
                }
                onClick={handleResendCode}
                className={`text-sm font-bold flex items-center gap-2 mx-auto transition-colors px-4 py-2 rounded-lg ${
                  countdown > 0 || rateLimitCountdown !== null
                    ? "text-slate-400 bg-slate-100 dark:bg-slate-800/50"
                    : "text-primary hover:bg-primary/5 hover:underline"
                }`}
              >
                <RefreshCw
                  className={`w-4 h-4 ${countdown > 0 || rateLimitCountdown !== null ? "opacity-50" : ""}`}
                />
                {rateLimitCountdown !== null
        ...
        {step === "newPassword" && (
          <>
            ...
            <p className="text-slate-500 dark:text-slate-400 mb-8 font-medium">
              Please select a strong password that you haven't used before.
            </p>
              ...
              {password.length > 0 && !allRequirementsMet && (
                <div className="p-4 bg-slate-50 dark:bg-slate-800/50 rounded-xl border border-slate-100 dark:border-slate-700 animate-in fade-in zoom-in-95 duration-200">
                  <div className="flex justify-between items-center mb-1">
                    ...
                    <span
                      className={`text-xs font-bold ${passwordStrength < 3 ? "text-yellow-500" : "text-green-500"}`}
                    >
                      {passwordStrength < 2
                        ? "Weak"
                        : passwordStrength === 3
                  ...
                  <div className="flex gap-2 h-1.5 mb-4">
                    {[1, 2, 3, 4].map((level) => (
                      <div
                        key={level}
                        className={`flex-1 rounded-full transition-all duration-500 ${passwordStrength >= level ? "bg-green-500" : "bg-slate-200 dark:bg-slate-700"}`}
                      />
                    ))}
                  </div>
                  <div className="space-y-2">
                    {requirements.map((req) => (
                      <div key={req.id} className="flex items-center gap-2">
                        <div
                          className={`w-4 h-4 rounded-full border flex items-center justify-center ${req.met ? "bg-green-500 border-green-500 text-white" : "border-slate-300"}`}
                        >
                          {req.met && (
                        ...
                        <span
                          className={`text-xs ${req.met ? "text-slate-700 dark:text-slate-200 font-bold" : "text-slate-400"}`}
                        >
                          {req.label}
                        </span>
                      </div>
