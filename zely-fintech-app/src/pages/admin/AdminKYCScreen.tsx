...
const AdminKYCScreen: React.FC = () => {
    ...
    const handleReject = async (id: string) => {
        if (rejectReason.length < 5 || rejectReason.length > 500) {
            showToast('Reason must be between 5 and 500 characters', 'error');
            return;
        }
        ...
        <div className="p-4 md:p-6 space-y-6">
            <div className="flex flex-col md:flex-row md:items-center justify-between gap-4">
                <div>
                    <h1 className="text-2xl font-bold text-gray-900 dark:text-white flex items-center gap-2">
                        <Shield className="text-blue-600" size={24} /> KYC Verification Center
                    </h1>
                    <p className="text-sm text-gray-500 dark:text-gray-400 mt-1">Review and manage identity verification requests</p>
                </div>
            ...
            {loading ? (
                ...
                <div className="bg-white dark:bg-gray-900 rounded-2xl border border-gray-200 dark:border-gray-800 p-12 text-center space-y-4 shadow-sm">
                    ...
                    <div>
                        <h3 className="text-xl font-bold text-gray-900 dark:text-white">Clean Queue!</h3>
                        <p className="text-gray-500 dark:text-gray-400 mt-1">There are no pending KYC submissions to review at this time.</p>
                    </div>
                </div>
            ) : (
                                ...
                                <tr key={sub.id} className="hover:bg-gray-50 dark:hover:bg-gray-800/20 transition-colors">
                                    <td className="px-6 py-4">
                                        <div className="flex items-center gap-3">
                                            ...
                                            <div>
                                                <div className="font-bold text-gray-900 dark:text-white">{sub.userName || 'Anonymous User'}</div>
                                    ...
                                    <td className="px-6 py-4">
                                        <span className={`px-2.5 py-1 rounded-full text-[10px] font-bold border ${
                                            sub.tier === 'TIER_3' ? 'bg-purple-100 dark:bg-purple-900/30 text-purple-700 border-purple-200' : 
                    ...
                    <div className="bg-white dark:bg-gray-900 rounded-3xl w-full max-w-4xl max-h-[90vh] overflow-hidden flex flex-col shadow-2xl border border-gray-100 dark:border-gray-800 animate-in zoom-in-95 duration-300">
                        ...
                        <div className="p-6 border-b border-gray-100 dark:border-gray-800 flex items-center justify-between bg-gray-50 dark:bg-gray-800/30">
                            <div className="flex items-center gap-4">
                                ...
                                <div>
                                    <h2 className="text-xl font-bold text-gray-900 dark:text-white">Review {selectedSub.tier.replace('_', ' ')} Application</h2>
                                    <p className="text-sm text-gray-500">Submitted by {selectedSub.userEmail} on {new Date(selectedSub.submittedAt).toLocaleDateString()}</p>
                                </div>
                            </div>
                            <button 
                                onClick={() => {
                            ...
                            <div className="grid grid-cols-1 md:grid-cols-2 gap-8">
                                ...
                                <div className="space-y-6">
                                    <h3 className="font-bold text-gray-900 dark:text-white flex items-center gap-2 border-b border-gray-100 dark:border-gray-800 pb-2">
                                        <UserIcon size={18} className="text-blue-600" /> Identity Information
                                    </h3>
                                        ...
                                        <div className="space-y-4">
                                            <div className="grid grid-cols-2 gap-4">
                                                <div className="space-y-1">
                                                    <p className="text-xs font-bold text-gray-500 uppercase">BVN</p>
                                                    <p className="font-mono text-gray-900 dark:text-white">{selectedSub.data.bvn}</p>
                                                </div>
                                                <div className="space-y-1">
                                                    <p className="text-xs font-bold text-gray-500 uppercase">NIN</p>
                                                    <p className="font-mono text-gray-900 dark:text-white">{selectedSub.data.nin}</p>
                                                </div>
                                                <div className="space-y-1">
                                                    <p className="text-xs font-bold text-gray-500 uppercase">DOB</p>
                                                    <p className="text-gray-900 dark:text-white">{selectedSub.data.dateOfBirth}</p>
                                                </div>
                                                <div className="space-y-1">
                                                    <p className="text-xs font-bold text-gray-500 uppercase">ID Type</p>
                                                    <p className="text-gray-900 dark:text-white">{selectedSub.data.governmentId.type.replace(/_/g, ' ')}</p>
                                                </div>
                                            </div>
                                            
                                            <div className="space-y-1 pt-2">
                                                <p className="text-xs font-bold text-gray-500 uppercase flex items-center gap-1"><MapPin size={12}/> Address</p>
                                                <p className="text-gray-900 dark:text-white">
                                                    {selectedSub.data.address.street}, {selectedSub.data.address.city}, {selectedSub.data.address.state}, {selectedSub.data.address.country}
                                                </p>
                                            </div>
                                        </div>
                                    ) : (
                                        <div className="p-4 bg-purple-50 dark:bg-purple-900/10 rounded-2xl border border-purple-100 dark:border-purple-800/30">
                                            <p className="text-sm font-medium text-purple-800 dark:text-purple-300">Biometric Tier 3 Submission</p>
                                            <p className="text-xs text-purple-700/70 dark:text-purple-400/70 mt-1">Review the files on the right to verify liveness and facial match.</p>
                                        </div>
                                    )}

                                    {/* Action History or similar can go here */}
                                    <div className="p-4 bg-gray-50 dark:bg-gray-800/50 rounded-2xl space-y-3">
                                        <h4 className="text-sm font-bold text-gray-700 dark:text-gray-300">Quick Audit</h4>
                                        <div className="flex items-center justify-between text-xs">
                                            <span className="text-gray-500">Risk Assessment</span>
                                            <span className="text-green-600 font-bold">LOW RISK</span>
                                        </div>
                                        <div className="flex items-center justify-between text-xs">
                                            <span className="text-gray-500">Auto-verification Search</span>
                                            <span className="text-gray-900 dark:text-white font-medium">Matching Record Found</span>
                                        </div>
                                    </div>
                                </div>
                                ...
                                <div className="space-y-6">
                                    <h3 className="font-bold text-gray-900 dark:text-white flex items-center gap-2 border-b border-gray-100 dark:border-gray-800 pb-2">
                                        <FileText size={18} className="text-blue-600" /> Supporting Documents
                                    </h3>
                                        ...
                                        {isTier2Data(selectedSub.data) ? (
                                            <>
                                                <div className="space-y-2">
                                                    <p className="text-xs font-bold text-gray-500 uppercase">Government ID Document</p>
                                                    <a 
                                                    ...
                                                    >
                                                        <div className="flex items-center justify-between">
                                                            <div className="flex items-center gap-3">
                                                                ...
                                                                <span className="text-sm font-semibold">View ID Card</span>
                                                            </div>
                                                ...
                                                <div className="space-y-2">
                                                    <p className="text-xs font-bold text-gray-500 uppercase">Proof of Address</p>
                                                    <a 
                                                    ...
                                                    >
                                                        <div className="flex items-center justify-between">
                                                            <div className="flex items-center gap-3">
                                                                ...
                                                                <span className="text-sm font-semibold">View Proof Document</span>
                                                            </div>
                                                            <ExternalLink size={16} />
                                                        </div>
                                                    </a>
                                            ...
                                            <>
                                                <div className="space-y-2">
                                                    <p className="text-xs font-bold text-gray-500 uppercase">Selfie Photo</p>
                                                    <a 
                                                    ...
                                                    >
                                                        <div className="flex items-center justify-between">
                                                            <div className="flex items-center gap-3">
                                                                ...
                                                                <span className="text-sm font-semibold">View Selfie Image</span>
                                                            </div>
                                                            <ExternalLink size={16} />
                                                        </div>
                                                    </a>
                                                ...
                                                <div className="space-y-2">
                                                    <p className="text-xs font-bold text-gray-500 uppercase">Liveness Video</p>
                                                    <a 
                                                        href={(selectedSub.data as Tier3Payload).livenessVideoUrl} 
                                                        target="_blank" 
                                                        rel="noreferrer"
                                                        className="block p-4 bg-gray-50 dark:bg-gray-800 rounded-2xl border border-gray-100 dark:border-gray-700 hover:border-blue-500 hover:text-blue-600 group transition-all"
                                                    >
                                                        <div className="flex items-center justify-between">
                                                            <div className="flex items-center gap-3">
                                                                ...
                                                                <span className="text-sm font-semibold">View Liveness Video</span>
                                                            </div>
                                                            <ExternalLink size={16} />
                                                        </div>
                                                    </a>
                            ...
                            {showRejectInput && (
                                <div className="bg-red-50 dark:bg-red-900/10 p-6 rounded-2xl border border-red-100 dark:border-red-900/30 space-y-4 animate-in slide-in-from-top-4">
                                    ...
                                    <div className="flex justify-end gap-3 text-xs">
                                        ...
                                        >
                                            {actionLoading === 'reject' && <Loader2 size={14} className="animate-spin" />} Confirm Rejection
                                        </button>
                                    </div>
                                </div>
                            )}
