...
import { User, Mail, Phone, MapPin, ShieldCheck, Upload, ChevronRight } from 'lucide-react';
...
const ProfileScreen: React.FC = () => {
    ...
    const { data: status, execute: fetchStatus } = useAsync<KYCStatusResponse>(kycService.getMyStatus);
                    ...
                    <div className="bg-white dark:bg-slate-900 rounded-[2rem] p-8 border border-slate-200 dark:border-slate-800 text-center shadow-sm">
                        ...
                        <div className="flex flex-col items-center gap-3">
                            <span className={`px-3 py-1 text-xs font-bold uppercase rounded-full flex items-center gap-1 ${
                                currentTier === 'TIER_3' ? 'bg-green-100 dark:bg-green-900/20 text-green-600 dark:text-green-400' :
                                currentTier === 'TIER_2' ? 'bg-blue-100 dark:bg-blue-900/20 text-blue-600 dark:text-blue-400' :
                                'bg-slate-100 dark:bg-slate-800 text-slate-600 dark:text-slate-400'
                            }`}>
                                <ShieldCheck className="w-3 h-3" /> 
                                {currentTier === 'TIER_3' ? 'KYC Tier 3' : currentTier === 'TIER_2' ? 'KYC Tier 2' : 'KYC Tier 1'}
                            </span>
