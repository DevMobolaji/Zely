...
const ProvisioningScreen: React.FC = () => {
  ...
  //     },
  //     {
  //       label: "Allocating ledger nodes...",
  //       icon: Database,
  //       status: "WALLETS_CREATED",
  //     },
  //     {
  //       label: "Syncing with regional gateways...",
  //       icon: Globe,
  ...
  const steps = [
    { label: "Verifying your identity...", icon: Shield },
    { label: "Setting up your accounts...", icon: Database },
    { label: "Finalizing your vault...", icon: Zap },
  ];
  ...
  useEffect(() => {
    const checkStatus = async () => {
      try {
        ...
        if (data.status === "EMAIL_VERIFIED") setStep(0);
        else if (data.status === "ACCOUNT_PROVISIONING") setStep(1);
      } catch (err) {
  ...
  const handleRetry = async () => {
    ...
    try {
      ...
      showToast("error", "Retry failed. Please contact support.");
      setError(true);
    }
  };
      ...
      <div className="max-w-md w-full text-center">
        {!error ? (
          <>
            ...
            <h1 className="text-3xl font-black mb-4 tracking-tight underline decoration-primary decoration-4 underline-offset-8">
              Setting Up Your Vault
            </h1>
            <p className="text-slate-500 mb-12">
              Please wait while we provision your decentralized financial
              infrastructure.
            </p>
            ...
            <div className="space-y-4 text-left max-w-sm mx-auto">
              {steps.map((s, i) => {
                ...
                return (
                  <div
                    key={i}
                    className={`flex items-center gap-4 p-4 rounded-2xl border transition-all duration-500 ${isDone ? "bg-green-50 dark:bg-green-900/20 border-green-100 dark:border-green-800" : isCurrent ? "bg-white dark:bg-slate-900 border-primary shadow-lg shadow-primary/10" : "bg-slate-50 dark:bg-slate-800/50 border-transparent opacity-50"}`}
                  >
                    <div
                      className={`w-10 h-10 rounded-xl flex items-center justify-center ${isDone ? "bg-green-500 text-white" : isCurrent ? "bg-primary text-white" : "bg-slate-200 dark:bg-slate-700 text-slate-400"}`}
                    >
                      {isDone ? (
                        <CheckCircle2 className="w-6 h-6" />
                      ) : (
                        <Icon
                          className={`w-6 h-6 ${isCurrent ? "animate-bounce" : ""}`}
                        />
                      )}
                    </div>
                    <div>
                      <p
                        className={`text-sm font-bold ${isDone ? "text-green-700 dark:text-green-400" : isCurrent ? "text-slate-900 dark:text-white" : "text-slate-400"}`}
                      >
                        {s.label}
                      </p>
                      {isCurrent && (
                        <div className="h-1 w-24 bg-primary/20 rounded-full mt-1 overflow-hidden">
          ...
          <div className="animate-in fade-in zoom-in duration-500">
            ...
            <p className="text-slate-500 mb-8 max-w-xs mx-auto">
              We encountered a problem while allocating your ledger nodes. This
              is often temporary.
            </p>
            <button
              onClick={handleRetry}
