...
import {
  IAuthRequest,
  IRequestContext,
} from "@/config/interfaces/request.interface";
...
export function attachRequestContext(
  ...
  if (!req.requestId) {
    throw new Error(
      "requestId missing. attachRequestId middleware must run first.",
    );
  }
...
export function getRequestContext(req: IAuthRequest): IRequestContext {
  ...
  if (!req.context) {
    throw new Error(
      "Request context not initialized. Did you forget attachRequestContext middleware?",
    );
  }
  return req.context;
}
